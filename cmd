    ...
    try:
        os.execvp(
            ...
            [
                "bash",
                "-c",
                'live -t "%s" -p "%s" > ./static/%s.svg'
                % (type_line, print_line, opt.filename),
            ],
        )
    ...
    try:
        os.execvp(
            ...
            [
                "bash",
                "-c",
                "for f in ./content/" + content_t + "/*.md ;do aspell check $f;done",
            ],
        )
    except Exception as e:
        print(e)
...
def new(_):
    ...
    try:
        res = run(
            ...
            % (
                os.listdir("./content")[
                    list(filter(lambda x: x[1] is True, values.items()))[0][0]
                ],
                values["Name"],
            )
        )
    ...
    if input("open editor? [y] or n:") != "n":
        ...
        if os.getenv("EDITOR") != "":
            res = subprocess.run(
                [
                    str(os.getenv("EDITOR")),
                    list(
                        filter(
                            lambda w: values["Name"] + ".md" in w[2],
                            os.walk("./content"),
                        )
                    )[0][0]
                    + "/"
